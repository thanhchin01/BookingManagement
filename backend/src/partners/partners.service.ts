...
export class PartnersService {
  ...
  async findAll(): Promise<PartnerProfile[]> {
    return this.prisma.partnerProfile.findMany({
      include: {
        user: true, // Bao gồm thông tin tài khoản người dùng liên kết
      },
  ...
  // Lấy hồ sơ đối tác theo User ID
  async findByUserId(userId: string | number | bigint): Promise<PartnerProfile> {
    const partner = await this.prisma.partnerProfile.findUnique({
      where: { userId: BigInt(userId) },
    });
    if (!partner) {
      throw new NotFoundException(`Tài khoản người dùng này chưa đăng ký hồ sơ đối tác.`);
    }
    return partner;
  }

  // Tạo/Đăng ký hồ sơ đối tác mới
  async create(data: any): Promise<PartnerProfile> {
    ...
    if (existing) {
      throw new ConflictException('Tài khoản người dùng này đã có hồ sơ đối tác.');
    }
  ...
  async updateStatus(id: string | number | bigint, status: string, approvedByAdminId?: string | number | bigint): Promise<PartnerProfile> {
