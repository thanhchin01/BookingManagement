...
export const PartnerFormModal: React.FC<PartnerFormModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal Card */}
      <div className="relative z-10 w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-md text-left">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Tên tổ hợp sân / Doanh nghiệp</label>
            <input 
              type="text"
              disabled={isSaving}
              value={businessName}
              ...
              className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 text-xs text-white px-3 py-2.5 rounded-xl focus:outline-none placeholder-slate-700 transition"
              required
            />
          </div>
          ...
          <div className="space-y-2.5 border-t border-slate-800/80 pt-4">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Cấu hình loại hình hoa hồng</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`flex items-center justify-center gap-2 py-3 px-4 rounded-xl border text-xs font-bold transition cursor-pointer select-none ${
                  commissionType === 'PERCENTAGE'
                    ? 'bg-slate-950 border-emerald-555 text-white shadow-inner shadow-emerald-500/10' 
                    : 'bg-slate-950/40 border-slate-850 text-slate-400 hover:border-slate-800 hover:text-slate-300'
                }`}
              >
                <Percent className={`w-4 h-4 ${commissionType === 'PERCENTAGE' ? 'text-emerald-400' : 'text-slate-550'}`} />
                Theo phần trăm (%)
              </button>
              ...
              >
                <Coins className={`w-4 h-4 ${commissionType === 'FIXED_AMOUNT' ? 'text-emerald-400' : 'text-slate-550'}`} />
                Số tiền cố định (đ)
              </button>
            </div>
          </div>
          ...
          {commissionType === 'PERCENTAGE' ? (
            <div className="space-y-1.5 animate-fade-in">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Tỉ lệ hoa hồng (%)</label>
              <div className="relative">
                <input 
                  ...
                  max={100}
                  step={0.5}
                  className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 text-xs text-white pl-3 pr-10 py-2.5 rounded-xl focus:outline-none placeholder-slate-700 transition font-mono font-bold"
                  required
                />
            ...
            <div className="space-y-1.5 animate-fade-in">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Số tiền hoa hồng cố định (VND)</label>
              <div className="relative">
                <input 
                  type="number"
                  disabled={isSaving}
                  ...
                  min={0}
                  step={1000}
                  className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 text-xs text-white pl-3 pr-12 py-2.5 rounded-xl focus:outline-none placeholder-slate-700 transition font-mono font-bold"
                  required
                />
                <div className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-xs select-none">
                  VND
          ...
          <div className="grid grid-cols-2 gap-3 mt-6 pt-4 border-t border-slate-800">
            <button
              ...
              disabled={isSaving}
              onClick={onClose}
              className="w-full py-2.5 bg-slate-850 hover:bg-slate-800 disabled:opacity-50 border border-slate-800 hover:border-slate-700 text-xs font-bold text-slate-350 rounded-xl transition cursor-pointer"
            >
              Hủy bỏ
            </button>
            <button
