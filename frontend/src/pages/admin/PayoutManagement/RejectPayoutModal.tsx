...
export const RejectPayoutModal: React.FC<RejectPayoutModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-955 bg-slate-950/60 backdrop-blur-xs p-4 animate-in fade-in duration-200">
      <form 
        onSubmit={onSubmit}
      ...
      >
        <div className="flex items-center gap-3 text-rose-455 text-rose-400">
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-slate-400 block">Lý do từ chối chính thức (Bắt buộc):</label>
          <textarea
            required
            ...
            className="w-full bg-slate-955 bg-slate-950 border border-slate-850 focus:border-rose-500 rounded-xl p-3 text-xs text-white placeholder-slate-500 outline-none transition resize-none h-24"
          />
        </div>
          ...
          <button
            ...
            className={`px-5 py-2 text-xs font-bold text-white border-0 rounded-xl transition cursor-pointer ${
              rejectReasonInput.trim() 
                ...
                : 'bg-slate-950 text-slate-655 text-slate-600 border border-slate-850 cursor-not-allowed'
            }`}
          >
            Xác Nhận Từ Chối
          </button>
