        ...
        <div className="lg:col-span-2 space-y-4">
          {filteredDisputes.length === 0 ? (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center text-slate-400 space-y-3 shadow-xs">
              ...
              <p className="text-[11px] text-slate-400 max-w-sm mx-auto">Hệ thống đang vận hành ổn định mượt mà, không có khiếu nại chưa xử lý trong danh mục này.</p>
            </div>
          ) : (
            filteredDisputes.map(d => (
              <div 
                ...
                className={`bg-slate-900 border rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 cursor-pointer transition hover:border-slate-700 shadow-lg ${
                  selectedDispute?.id === d.id ? 'border-emerald-500 bg-slate-900' : 'border-slate-800'
                }`}
              >
                <div className="space-y-3 flex-grow">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-xs font-mono font-bold text-slate-400">{d.id}</span>
                    <span className="text-[9px] bg-slate-950 border border-slate-850 px-2 py-0.5 rounded text-amber-400 font-bold uppercase tracking-wider">
                ...
                <div className="w-full sm:w-auto flex sm:flex-col items-end justify-between sm:justify-center gap-2.5 border-t sm:border-t-0 border-slate-850 pt-3 sm:pt-0 shrink-0">
                  <span className="text-[9px] text-slate-400 font-mono">{d.createdAt}</span>
                  <button className="flex items-center gap-1 text-[10px] font-extrabold text-slate-300 hover:text-white bg-slate-950 hover:bg-slate-800 border border-slate-850 px-3 py-1.5 rounded-xl transition duration-150">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-5 sticky top-24 shadow-lg text-slate-100">
              ...
              <div className="border-b border-slate-800 pb-3 space-y-1.5">
                <div className="flex justify-between items-baseline">
                  <span className="text-[9px] font-mono text-slate-400">{selectedDispute.id}</span>
                  <span className="text-[9px] text-slate-400">{selectedDispute.createdAt}</span>
                </div>
              ...
              <div className="space-y-1.5 text-xs text-left">
                <span className="text-slate-400 font-bold block uppercase tracking-wider">Nội dung tường trình của Khách:</span>
              ...
              {selectedDispute.evidence.length > 0 && (
                <div className="space-y-1.5 text-xs text-left">
                  <span className="text-slate-400 font-bold block uppercase tracking-wider">Tệp tin bằng chứng đính kèm:</span>
              ...
              {selectedDispute.status === 'OPEN' && (
                <button
                  ...
                  className="w-full py-3 bg-amber-500/10 border border-amber-500/20 text-amber-400 hover:bg-amber-500/25 text-xs font-bold rounded-xl cursor-pointer transition flex items-center justify-center gap-1"
                >
              ...
              {selectedDispute.status === 'INVESTIGATING' && (
                ...
                >
                  ...
                  <div className="space-y-1.5 text-xs">
                    <label className="text-slate-400 block">Quyết định phán quyết chung cuộc:</label>
                    <select
                      value={verdict}
                    ...
                    >
                      <option value="REFUND" className="bg-slate-950 text-white">Khách hàng thắng: Hoàn 100% tiền đơn đặt</option>
                      <option value="PARTNER_WIN" className="bg-slate-950 text-white">Chủ sân thắng: Giữ nguyên doanh thu đối tác</option>
                    </select>
                  </div>

                  {/* Resolution text */}
                  <div className="space-y-1.5 text-xs">
                    <label className="text-slate-400 block">Ghi chú giải pháp chính thức:</label>
                    <textarea
                      required
                      value={resolutionText}
                      onChange={(e) => setResolutionText(e.target.value)}
                      placeholder="Mô tả lại căn cứ hòa giải, cuộc gọi xác minh giữa 2 bên để lưu lịch sử..."
                      className="w-full bg-slate-950 border border-slate-850 focus:border-emerald-500 rounded-xl p-3 text-xs text-white outline-none transition resize-none h-20"
                    />
                  </div>

                  <button
                    type="submit"
                    className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-xs font-bold text-white border-0 rounded-xl cursor-pointer shadow-lg shadow-emerald-600/20 transition active:scale-95"
                  >
              ...
              {selectedDispute.status === 'RESOLVED' && (
                <div className="space-y-3.5 border-t border-slate-800 pt-4 text-xs text-left">
                  ...
                  <div className="space-y-1">
                    <span className="text-slate-400 font-bold block uppercase tracking-wider">Nội dung hòa giải ghi chú:</span>
                    <p className="text-slate-300 leading-relaxed bg-slate-950 border border-slate-850 p-3 rounded-xl m-0">
