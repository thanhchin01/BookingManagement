...
export const AdminSearchFilter: React.FC<AdminSearchFilterProps> = ({
  ...
  return (
    ...
    >
      {/* 1. Ô TÌM KIẾM */}
      <div 
        className={`flex items-center gap-3 bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2 w-full transition focus-within:border-teal-500 focus-within:bg-slate-950 ${
      ...
      >
        ...
        <input 
          ...
          className="bg-transparent border-0 text-xs text-white focus:outline-none placeholder-slate-500 w-full"
        />
      </div>

      {/* 2. BỘ LỌC (Chỉ hiển thị khi có tham số cấu hình) */}
      {hasFilter && (
        <div className="flex items-center bg-slate-950 border border-slate-800 rounded-xl px-2 py-1 transition focus-within:border-teal-500 focus-within:bg-slate-950">
          <span className="text-[10px] text-slate-400 font-bold uppercase px-2 shrink-0">{filterLabel}</span>
          <select 
            value={filterValue}
