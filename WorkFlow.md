...
erDiagram
    ...
    partner_profiles ||--o| partner_payouts : "1:N partner_id"
    admins ||--o| partner_payouts : "1:N processed_by"
...
| `commission_rate` | decimal(5,2) | Default: 10.00 | Phần trăm phí hoa hồng sàn thu trên mỗi đơn đặt lịch thành công |
| `balance` | decimal(15,2) | Default: 0.00 | Số dư ví tiền hiện tại mà đối tác có thể gửi yêu cầu rút |
...
##### 25. Bảng `partner_payouts` — Yêu cầu rút tiền & Đối soát tài chính
Bảng điều phối dòng tiền ra từ sàn chuyển về tài khoản ngân hàng của Đối tác sau khi đã trừ tỷ lệ hoa hồng chiết khấu.
| Tên trường (Field) | Kiểu dữ liệu | Thuộc tính / Ràng buộc | Giải thích ý nghĩa (Description) |
| :--- | :--- | :--- | :--- |
| `id` | bigint | Primary Key, Auto Increment | Định danh lệnh chuyển khoản |
| `partner_id` | bigint | FK (`partner_profiles`), Not Null | Đối tác yêu cầu rút tiền |
| `amount` | decimal(15,2) | Not Null | Tổng số doanh thu tích lũy trong kỳ thanh toán được duyệt |
| `commission_deducted`| decimal(15,2) | Not Null | Số tiền phí dịch vụ sàn giữ lại (Ví dụ: `amount * commission_rate`) |
| `net_amount` | decimal(15,2) | Not Null | Số tiền thực nhận đối tác được chuyển (`amount - commission_deducted`) |
| `period_start` | date | Nullable | Ngày bắt đầu tính chu kỳ đối soát doanh thu |
| `period_end` | date | Nullable | Ngày kết thúc chu kỳ đối soát doanh thu |
| `bank_account` | varchar(50) | Nullable | Số tài khoản ngân hàng thụ hưởng của đối tác |
| `bank_name` | varchar(100) | Nullable | Tên ngân hàng và chi nhánh giao dịch thụ hưởng |
| `status` | varchar(20) | Default: 'PENDING' | Trạng thái giao dịch: `PENDING` (Chờ duyệt), `PROCESSING` (Đang chuyển khoản), `COMPLETED` (Đã chuyển thành công), `FAILED` (Giao dịch lỗi) |
| `processed_by` | bigint | Nullable, FK (`admins`) | Admin duyệt lệnh rút tiền và thực hiện chuyển khoản |
| `created_at` | timestamp | Nullable | Thời điểm đối tác nhấn nút gửi yêu cầu rút tiền |
...
### 2. Luồng Thiết lập Chi nhánh, Dịch vụ & Giá (Venue Setup Flow)

```
[Thêm Chi nhánh (Location)] ──> [Chọn Tỉnh/Thành, Nhập GPS]
            │
            └──> [Thêm Dịch vụ (Service)] ──> [Chọn Thể loại, Nhập giá cơ bản]
                         │
                         └──> [Cấu hình Time Slots] ──> [Thêm khung giờ vàng x1.5, giờ vắng x0.8]
                         │
                         └──> [Cấu hình Overrides] ──> [Cài đặt nghỉ lễ Tết hoặc đổi giá ngày đặc biệt]
```

*   **Bước 1:** Đối tác thực hiện tạo thông tin chi nhánh kinh doanh mới (`locations`), xác định địa giới hành chính chính xác nhờ kết nối các bảng dữ liệu Tỉnh/Thành phố (`location_provinces`) và Quận/Huyện/Phường/Xã (`location_wards`).
*   **Bước 2:** Với mỗi chi nhánh, đối tác tạo các dịch vụ cụ thể (`services`) như Sân cầu lông số 1, Sân bóng đá mini 5 người. Mỗi dịch vụ có `base_price_per_hour` làm mức giá gốc.
*   **Bước 3:** Đối tác cài đặt khung giờ hoạt động hàng tuần (`time_slots`) xác định giờ mở cửa/đóng cửa và hệ số điều chỉnh giá `price_modifier` (Ví dụ: Giờ trưa 12h-13h hệ số x0.8; Giờ vàng 18h-20h hệ số x1.5).
*   **Bước 4:** Đối tác cài đặt ngày đặc biệt (`slot_overrides`) như ngày Quốc Khánh, Tết Dương Lịch để tự động khóa sân (`is_closed = true`) hoặc tăng hệ số giá đột xuất nhằm tối ưu doanh thu.
*   **Bước 5:** Đối tác đăng bán các sản phẩm phụ trợ (`products`) tại chi nhánh như nước suối, thuê áo bib, thuê vợt cầu lông để chuẩn bị bán kèm cho khách hàng đặt cùng giờ.
...
[Admin ra quyết định] <── [Admin điều tra liên hệ 2 bên] <────────┘
          ...
          └───> [Phán quyết Chủ sân thắng: Xác nhận giao dịch, ghi nhận số dư ví đối tác]
                                                                  │
[Thực hiện chuyển khoản & Hoàn tất] <── [Admin duyệt lệnh] <── [Chủ sân yêu cầu rút tiền]
```
...
*   **Bước 4:** Định kỳ cuối tuần/tháng hoặc khi số dư đạt hạn mức tối thiểu, chủ sân gửi yêu cầu rút tiền về tài khoản ngân hàng (`PortalController`).
*   **Bước 5:** Yêu cầu tạo bản ghi `partner_payouts` (status: `PENDING`). Admin truy cập mục Quản lý Tài chính để kiểm duyệt, thực hiện chuyển tiền ngân hàng và bấm **Xác nhận hoàn thành** cập nhật status thành `COMPLETED`, ghi nhận người xử lý `processed_by`.
